...
//   if (!store.state.cartProductsData.length) {
//     store.state.cartProductsData.push(element);
//   } else if (
//     store.state.cartProductsData.find((item) => item.id === element.id)
//   ) {
//     store.state.cartProductsData.element.quantity += 1;
//   } else {
//     store.state.cartProductsData.push(element);
//   }
// }
function addToCart(element) {
  const existingProduct = store.state.cartProductsData.find(
  ...
  if (existingProduct) {
    ...
    store.state.cartProductsData.push(element); // Add the new product to the cart
  }
}
</script>
...
<template>
  <body>
    ...
    <main class="container">
      <div class="catalog">
        <h2 class="catalog__title">Наушники</h2>
        <ul class="catalog__list flex">
          <li
            class="catalog__item product"
            v-for="product in store.state.headphones"
            :key="product.id"
          >
            ...
            <div class="product__info flex">
              <div class="product__info-left">
                ...
                <p class="product__score">
                  <svg
                    class="product__score-img"
                    width="25"
                    height="23"
                    viewBox="0 0 25 23"
                    fill="none"
              ...
              <div class="product__info-right product__price">
                <p class="product__price-new">{{ product.price.new }} ₽</p>
                <span class="product__price-old" v-if="product.price.old"
                  >{{ product.price.old }} ₽</span
                >
                <p class="product__buy" @click="addToCart(product)">Купить</p>
              </div>
            </div>
          </li>
        </ul>
      </div>
      <h2 class="catalog__title">Беспроводные наушники</h2>
      <ul class="catalog__list flex">
        <li
          class="catalog__item product"
          v-for="product in store.state.wirelessHeadphones"
          :key="product.id"
        >
          <a class="product__link" href="#">
            <img
          ...
          <div class="product__info flex">
            ...
            <div class="product__info-right product__price">
              <span class="product__price-new">{{ product.price.new }} ₽</span>
              <p class="product__buy" @click="addToCart(product)">Купить</p>
            </div>
          </div>
        </li>
    ...
    <footer class="container">
      <div class="footer flex">
        <div class="footer__logo logo">QPICK</div>
        <ul class="footer__menu menu">
          <li class="menu__item">
            <a class="menu__link" href="#">Избранное</a>
          </li>
          <li class="menu__item">
            <router-link to="cart" class="menu__link" href="#"
              >Корзина</router-link
            >
          </li>
          <li class="menu__item">
